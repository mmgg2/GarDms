	...
	*/
	private function _getAuthModes(){
		if(CrugeUtil::config()->_lazyAuthModes == null){
			...
			if($f != '.' && $f != '..')
			{
				Yii::log("findfile: ".$f.", isPhp?".(CrugeUtil::isPhpFile($f)?"YES":"NO"),"info");
				if(CrugeUtil::isPhpFile($f)){
					...
					if(class_exists($className)){
						...
						if(self::isAuthMethodAvailable($inst->authName())==true){
							...
							//
							Yii::log("Clase de autenticacion encontrada [{$className}] "
						...
						Yii::log("clase de autenticacion no es valida class_exist retorna false. "
							.className,"error");
